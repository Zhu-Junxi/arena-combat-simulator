...
export function createSelectionView({ state, characters, categories, elements, i18n, getFighterSettings = null }) {
  ...
  function formatStat(value, cooldown = false) {
    ...
    const content = text + (cooldown ? `<small>${t('unit.seconds_short')}</small>` : '');
  ...
  function renderPanel(side) {
    ...
    const portrait = '<button class="portrait' + (character.art ? ' has-art' : '') + `" data-side="${side}" ` +
      `aria-label="${t('selection.choose_for', { side: translatedSide })}">` +
    ...
    const info = `<div class="info"><h2>${name}</h2><button class="avatar${character.art ? ' has-art' : ''}" data-side="${side}" ` +
      `aria-label="${t('selection.switch_to', { side: translatedSide })}">` +
      ...
      `<div><dt>${t('stats.attack')}</dt><dd>${formatStat(stats.attack)}</dd></div>` +
      `<div><dt>${t('stats.cooldown')}</dt><dd>${formatStat(stats.attackCD, true)}</dd></div>` +
      `<div><dt>${t('stats.speed')}</dt><dd>${formatStat(stats.speed)}</dd></div>` +
      `<div><dt>${t('stats.health')}</dt><dd>${formatStat(stats.health)}</dd></div></dl>${renderTrait(character, side)}</div>`;
    ...
    panel.innerHTML = panelDecoration(side) + `<div class="panel-head"><button class="side-button" data-side="${side}" aria-pressed="${active}">` +
  ...
  function renderRoster() {
    elements.roster.innerHTML = visibleCharacters().map(character => {
      const name = characterName(character);
      return `<button class="character" data-character="${character.id}" aria-label="${t('selection.character_aria', { name })}" aria-pressed="false">` +
  ...
  function render() {
    ...
    elements.categories.innerHTML = categories.map(category => {
      ...
      return `<button class="category" data-category="${category}" aria-label="${name}" aria-pressed="false"><span>${name}</span><small>${count}</small></button>`;
    }).join('');
    renderPanel('left');
    renderPanel('right');
    renderRoster();
